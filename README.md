...
| `repo-guard.cloudoperators.dev/removeOrganizationMember` | "true"/"false" | Allows the controller to remove org members who are not in any GitHub team. Requires all team-member lists to be fetchable; if none succeed, no removals are generated (safety rail). See `spec.protectedMembers` to exempt specific logins. | Disabled (must be "true" to remove) |
