...
| `repo-guard.cloudoperators.dev/removeOrganizationMember` | "true"/"false" | Allows the controller to remove org members who are not in any GitHub team. Requires at least one team-member list to be successfully fetched; if none succeed (all fetches fail), no removals are generated (safety rail). See `spec.protectedMembers` to exempt specific logins. | Disabled (must be "true" to remove) |
