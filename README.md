...
| `repo-guard.cloudoperators.dev/removeRepositoryDirectCollaborator` | "true"/"false" | Allows the controller to remove all direct repository collaborators, so that repository access is managed exclusively through teams. Org owners and `spec.protectedMembers` logins are exempt. | Disabled (must be "true" to remove) |
