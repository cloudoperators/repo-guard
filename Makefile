...
IMG ?= ghcr.io/cloudoperators/repo-guard:1.6.0-dev1
...
verify-manifests: ## Verify Helm CRDs are in sync with config/crd/bases
	git diff --exit-code -- charts/repo-guard/crds || (echo "Helm CRDs are not in sync with config/crd/bases. Run 'make manifests'." && exit 1)
...
controller-test: manifests generate fmt vet envtest ## Run tests.
...
.PHONY: e2e-up
e2e-up: ## Create local k3d cluster for e2e
	bash hack/e2e/e2e.sh up
...
e2e-install-crds: ## Install only repo-guard CRDs into the cluster
	bash hack/e2e/e2e.sh install-crds
...
helm: manifests kustomize helmify
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default | $(HELMIFY) -crd-dir charts/repo-guard

helm-push:
	cd charts/repo-guard && helm package . && helm push *.tgz oci://ghcr.io/cloudoperators/greenhouse-extensions/charts/repo-guard && rm *.tgz
