...
E2E_TRIGGER_LABEL_KEY=${E2E_TRIGGER_LABEL_KEY:-repo-guard.cloudoperators.dev/trigger}
...
cmd_test() {
  ...
  for crd in githubs.repo-guard.cloudoperators.dev githuborganizations.repo-guard.cloudoperators.dev githubteams.repo-guard.cloudoperators.dev githubteamrepositories.repo-guard.cloudoperators.dev githubaccountlinks.repo-guard.cloudoperators.dev staticmemberproviders.repo-guard.cloudoperators.dev ldapgroupproviders.repo-guard.cloudoperators.dev genericexternalmemberproviders.repo-guard.cloudoperators.dev; do
...
run_teams_scenario() {
  ...
  log_step "Setting label repo-guard.cloudoperators.dev/removeUser=false on GithubTeam/${GT1_NAME} in ${ns}"
  kubectl -n "${ns}" patch githubteam "$GT1_NAME" --type merge -p '{"metadata":{"labels":{"repo-guard.cloudoperators.dev/removeUser":"false"}}}'
  ...
  log_step "Setting label repo-guard.cloudoperators.dev/addUser=false on GithubTeam/${GT1_NAME} in ${ns}"
  kubectl -n "${ns}" patch githubteam "$GT1_NAME" --type merge -p '{"metadata":{"labels":{"repo-guard.cloudoperators.dev/addUser":"false"}}}'
  ...
  kubectl -n "${ns}" patch githubteam "$GT1_NAME" --type merge -p '{"metadata":{"labels":{"repo-guard.cloudoperators.dev/addUser":"true","repo-guard.cloudoperators.dev/removeUser":"true"}}}'
  kubectl -n "${ns}" patch team "$TEAM1" --type=merge --subresource=status -p "$(jq -nc --arg u1 "$USER1" --arg u2 "$USER2" '{status:{members:[{id:$u1, email:($u1+"@example.com"), firstName:$u1, lastName:$u1},{id:$u2, email:($u2+"@example.com"), firstName:$u2, lastName:$u2}], statusConditions: {conditions: []}}}')"
  wait_for_members_count "$GT1_NAME" 2 "${ns}"
