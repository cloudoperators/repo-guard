...
- Generates chart values from `internal/controller/test.env` (script: `hack/e2e/gen-values.sh`):
  ...
  - Adds `repoguard.cloudoperators.dev/skippedTTL` label support via chart values
...
- `e2e-test` runs, in order:
  ...
  3) Teams scenario (Greenhouse-driven):
     ...
     - Toggles `repoguard.cloudoperators.dev/addUser` / `repoguard.cloudoperators.dev/removeUser` labels and verifies expected behavior
